...
require_once './framework/class/hongbao.class.php';
...
if(!empty($xml)){
	// pdo_insert('test',['text'=>$xml,'time'=>time()]);
	//引入红包类
	$hongbao = new Hongbao();
	$data =  $hongbao->xmlToarray($xml);
	//如果是关注则发红包
	if(isset($data['EVENTKEY'])){
		$openid = $data['FROMUSERNAME'];
		$parameter = $data['EVENTKEY'];
		//首次关注
		if($data['EVENT'] == 'subscribe'){
			//得到红包码和红包id

			$code = $hongbao->mysubstr('qrscene_','--',$parameter);
			$id = $hongbao->mysubstr('--','',$parameter);

		}//已经关注过了得到红包码方式不一样,所以分开些
		elseif($data['EVENT'] == 'SCAN'){
			//得到红包码和红包id
			$code = $hongbao->mysubstr('','--',$parameter);
			$id = $hongbao->mysubstr('--','',$parameter);
		}
		
		if(isset($code ) && isset($id)){
			//得到红包码和id唯一识别后去发红包方法
			$hongbao->sendRedBag($code, $id,$openid);
		}
	}
}
//end
